...
taServer_API_Version = "V 260427 0915"  
# V 260427 0915 : 在 _handle_bot_build() 裏面，增加了對於下載的 zip 檔案的處理，會先把 zip 檔案下載到一個臨時位置，然後解壓縮到另一個臨時目錄，最後根據檔案名稱把需要的檔案複製到正確的位置，並且啟動對應的服務。這樣可以確保在處理 bot build 的過程中，不會直接操作原始檔案，增加了安全性和穩定性。  
...
def _activate_gateway_service_by_target(target_path: str) -> tuple[bool, str]:
    ...
    if active:
        action_cmd = ["restart", service_name]
        action_desc = "restart"
    else:
        action_cmd = ["start", service_name]
        action_desc = "start"

    _self_update_print(f"service action cmd: systemctl {' '.join(action_cmd)}")
...
_BOT_BUILD_ROOT_FILES = {"openclaw.json"}
_BOT_BUILD_AGENT_FILES = {"auth-profiles.json", "auth-state.json", "models.json"}
...
def _handle_bot_build(action_value: str | None, target_fname: str | None) -> tuple[bool, str]:
    ...
    extract_tmp = target_dir.parent / f"{target_dir.name}_bot_build_{ts}_extracted"
    ...
    try:
        ...
        file_count = _extract_zip_to_target(zip_tmp, extract_tmp)
        _self_update_print(f"bot build extracted files={file_count} to temp={extract_tmp}")

        # Route extracted files to correct destinations:
        #   openclaw.json          -> target_dir/
        #   auth-*.json / models.json -> target_dir/agents/main/agent/
        agent_dir = target_dir / "agents" / "main" / "agent"
        target_dir.mkdir(parents=True, exist_ok=True)
        agent_dir.mkdir(parents=True, exist_ok=True)

        copied = 0
        for src_file in extract_tmp.rglob("*"):
            if not src_file.is_file():
                continue
            name = src_file.name
            if name in _BOT_BUILD_ROOT_FILES:
                dest = target_dir / name
                shutil.copy2(src_file, dest)
                copied += 1
                _self_update_print(f"bot build copy {name} -> {dest}")
            elif name in _BOT_BUILD_AGENT_FILES:
                dest = agent_dir / name
                shutil.copy2(src_file, dest)
                copied += 1
                _self_update_print(f"bot build copy {name} -> {dest}")

        _self_update_print(f"bot build routed files={copied} target={target_dir}")

        ok, service_msg = _activate_gateway_service_by_target(normalized_target)
        if not ok:
            return False, service_msg

        return True, f"bot build success ({copied} files), {service_msg}"
